...
import {
  ...
  getLocalOutBalance,
  getTokenBalance,
} from "../../utils/bridge/getOutBalance";
...
import {
  ...
  RegisterAddress,
  removeLocalConfig,
} from "../../utils/bridge/getServerInfo";
...
function getInitialSwapState(state) {
  let wdInit =
    sessionStorage.getItem("WITHDRAW_HISTORY") &&
    sessionStorage.getItem("WITHDRAW_HISTORY") !== "undefined"
  ...
  let dpInit =
    sessionStorage.getItem("DEPOSIT_HISTORY") &&
    sessionStorage.getItem("DEPOSIT_HISTORY") !== "undefined"
  ...
  return {
    ...
    outputCurrency: state.outputCurrencyURL
      ? state.outputCurrencyURL === config.symbol
        ? !state.inputCurrencyURL ||
          (state.inputCurrencyURL && state.inputCurrencyURL !== config.symbol)
          ? config.symbol
          : ""
...
function swapStateReducer(state, action) {
  switch (action.type) {
    ...
    case "UPDATE_SWAPREGISTER": {
      if (
        action.token &&
        state.inputCurrency.toLowerCase() !== action.token.toLowerCase()
      ) {
        return state;
      }
      return {
        ...
        PlusGasPricePercentage: action.PlusGasPricePercentage
          ? action.PlusGasPricePercentage
          : "",
        ...
        redeemBigValMoreTime: action.redeemBigValMoreTime
          ? action.redeemBigValMoreTime
          : "",
        depositBigValMoreTime: action.depositBigValMoreTime
...
function formatOutName(name, extendObj) {
  let srcChainId =
    extendObj && extendObj.BRIDGE && extendObj.BRIDGE.length > 0
...
const CrossBridge = (props) => {
  ...
  const fetchPoolTokens = useCallback(() => {
    ...
    if (extendObj && extendObj.APPROVE && account) {
      ...
      getAllowanceInfo(account, extendObj.APPROVE, chainId, inputCurrency).then(
        (res) => {
          // console.log(res)
          if (res.msg === "Success") {
  ...
  useEffect(() => {
    ...
    if (account && initIsDeposit && initIsRedeem) {
      getServerInfo(account, tokenOnlyOne, inputSymbol, chainId, version).then(
        (res) => {
          console.log(res);
          if (res.msg === "Success" && res.info) {
            ...
            try {
              ...
              if (!isSpecialCoin(coin)) {
                let erc20Token =
                  BridgeTokens[node] &&
                  BridgeTokens[node][coin] &&
                  BridgeTokens[node][coin].token
                    ? BridgeTokens[node][coin].token
                    : "";
                if (
                  initDepositAddress.toLowerCase() !==
                    serverInfo.depositAddress.toLowerCase() ||
                  tokenOnlyOne.toLowerCase() !==
                    serverInfo.token.toLowerCase() ||
                  (erc20Token &&
                    erc20Token.toLowerCase() !==
                      serverInfo.outnetToken.toLowerCase())
                ) {
                  console.log(1);
  ...
  useEffect(() => {
    ...
    let coin = formatCoin(inputSymbol);
    if (account && initIsDeposit && initIsRedeem) {
      RegisterAddress(account, tokenOnlyOne, coin, chainId, version).then(
        (res) => {
  ...
  const setOutBalance = useCallback(() => {
    ...
    if (node && account) {
      ...
      if (lob && lob.info) {
        let bl = amountFormatter(
          new BigNumber(lob.info.balance),
          inputDecimals,
          Math.min(8, inputDecimals)
        );
        setOutNetBalance(bl);
      } else {
        setOutNetBalance("");
  ...
  function formatBalance(value) {
    return `Balance: ${formatDecimal(
      value,
      Math.min(config.keepDec, inputDecimals)
    )}`;
  }
    ...
    if (bridgeType && bridgeType === "redeem") {
      if (
        ...
        if (
          ...
          isBTCAddress(recipient.address, inputSymbol)
        ) {
          if (
            extendObj &&
            extendObj.APPROVE &&
  ...
  function sendTxns(node) {
    ...
    let amountVal = ethers.utils.parseUnits(
      inputValueFormatted.toString(),
      inputDecimals
    );
    ...
    if (config.supportWallet.includes(walletType)) {
      ...
      setHardwareTxnsInfo(
        amountFormatter(amountVal, inputDecimals, inputDecimals) +
          " " +
          inputSymbol
      );
  ...
  const updateHashStatus = useCallback(() => {
    if (hashArr.length > 0) {
      for (let i = 0, len = hashArr.length; i < len; i++) {
        if (hashArr[i].chainID && hashArr[i].chainID !== config.chainID)
          continue;
        if (
          !hashArr[i].status ||
  ...
  const updateWithdrawStatus = useCallback(() => {
    if (withdrawArr.length > 0) {
      for (let i = 0, len = withdrawArr.length; i < len; i++) {
        if (withdrawArr[i].chainID && withdrawArr[i].chainID !== config.chainID)
          continue;
        if (
          ...
          getWithdrawHashStatus(
            ...
            // console.log(res)
            if (
              withdrawArr[res.index] &&
              res.hash === withdrawArr[res.index].hash
            ) {
              withdrawArr[res.index].status = res.status ? res.status : 0;
              withdrawArr[res.index].swapHash = res.swapHash
                ? res.swapHash
                : "";
              withdrawArr[res.index].swapStatus = res.swapStatus
                ? res.swapStatus
                : "pending";
              withdrawArr[res.index].swapTime = res.swapTime
                ? res.swapTime
                : "";
              dispatchSwapState({
                type: "UPDATE_WITHDRAW_STATUS",
  ...
  function txnsListDtil() {
    ...
    if (bridgeType === "redeem") {
      ...
      if (isSpecialCoin(mintDtil.coin)) {
        hashOutObj = {
          ...
          url:
            config[formatCoin(mintDtil.coin).toLowerCase()].lookHash +
            mintDtil.swapHash,
        };
      } else {
        hashOutObj = {
      ...
      <HashStatus
        ...
        {!mintDtil.status ? (
          <span className='green'>
            {bridgeType === "redeem" ? "Redeeming" : "Pending"}
          </span>
        ) : (
          ""
        )}
    ...
    return (
      <MintDiv>
        <MintList>
          <MintListLabel>
            {(bridgeType === "redeem" ? curNodeName : outNodeName) +
              " " +
              t("hash")}
            :
          </MintListLabel>
        ...
        {hashOutObj.hash ? (
          <MintList>
            <MintListLabel>
              {(bridgeType === "redeem" ? outNodeName : curNodeName) +
                " " +
                t("hash")}
              :
            </MintListLabel>
        ...
        {mintDtil.swapStatus ? (
          <HashStatus
            className={
              mintDtil.swapStatus === "confirming" ||
              mintDtil.swapStatus === "minting"
                ? "yellow"
                : mintDtil.swapStatus === "failure" ||
                  mintDtil.swapStatus === "timeout"
                ? "red"
                : "green"
            }>
            <div>
        ...
        <Button
          ...
          onClick={() => {
            ...
            if (isSpecialCoin(inputSymbol)) {
              ...
              setMintSureBtn(true);
              setHardwareTxnsInfo(
                inputValueFormatted + " " + formatCoin(inputSymbol)
              );
              setIsHardwareTip(true);
    ...
    <Page networkSensitive={false}>
      <Row>
        <Col xs={12} lg={{ span: 8, offset: 2 }}>
          ...
          <Header>
            <Title>
              {bridgeType && bridgeType === "redeem" ? "Redeem" : "Deposit"}
            </Title>
            ...
            <Modal.Body>
              <MintDiv>
                <MintList>
                  ...
                  <MintListVal>
                    {mintHistory && mintHistory?.mintHash
                      ? mintHistory?.mintHash
                      : ""}
                  </MintListVal>
                </MintList>
                <MintList>
                ...
                <MintList>
                  <MintListLabel>{t("value")}:</MintListLabel>
                  <MintListVal>
                    {mintHistory && mintHistory?.mintValue
                      ? mintHistory?.mintValue
                      : ""}
                  </MintListVal>
                </MintList>
                <MintList>
          ...
          <Modal
            show={mintDtilView}
            onHide={setMintDtilView.bind(this, false)}
            centered>
            <Modal.Header>
          ...
          <Card>
            <BridgeInputPanel
              ...
              onUserInput={(inputValue) => {
                ...
                let inputVal =
                  iValue && Number(iValue)
                    ? new BigNumber(
                        ethers.utils
                          .parseUnits(iValue.toString(), inputDecimals)
                          .toString()
                      )
                    : new BigNumber(0);
                let _fee = inputVal
                  .times(
                    ethers.utils.parseUnits(dFee.toString(), 18).toString()
                  )
                  .dividedBy(
                    new BigNumber(10).pow(new BigNumber(18)).toString()
                  );
                let _minFee = new BigNumber(
                  ethers.utils
                    .parseUnits(dMinFee.toString(), inputDecimals)
                    .toString()
                );
                let _maxFee = new BigNumber(
                  ethers.utils
                    .parseUnits(dMaxFee.toString(), inputDecimals)
                    .toString()
                );
                if (bridgeType && bridgeType === "redeem") {
                  _fee = inputVal
                    .times(
                      ethers.utils.parseUnits(fee.toString(), 18).toString()
                    )
                    .dividedBy(
                      new BigNumber(10).pow(new BigNumber(18)).toString()
                    );
                  _minFee = new BigNumber(
                    ethers.utils
                      .parseUnits(minFee.toString(), inputDecimals)
                      .toString()
                  );
                  _maxFee = new BigNumber(
                    ethers.utils
                      .parseUnits(maxFee.toString(), inputDecimals)
                      .toString()
                  );
                }
                if (_fee.isZero()) {
                ...
                if ((inputVal || inputVal === 0) && inputValue !== "") {
                  inputVal = amountFormatter(
                    inputVal,
                    inputDecimals,
                    Math.min(10, inputDecimals)
                  );
                } else {
                  inputVal = "";
                }
                dispatchSwapState({
                  ...
                  payload: {
                    ...
                    realyValue: inputVal
                      ? Number(Number(inputVal).toFixed(8))
                      : "",
                  },
                });
              }}
              ...
              label={
                bridgeType && bridgeType !== "redeem" ? "Deposit" : "Redeem"
              }
              ...
              extraText={
                bridgeType && bridgeType === "redeem" && inputBalanceFormatted
                  ? outNetBalance && !isSpecialCoin(inputSymbol)
                    ...
                    : ""
                  : inputBalanceFormatted &&
                    formatBalance(inputBalanceFormatted)
              }
              onCurrencySelect={(inputCurrency) => {
                dispatchSwapState({
                  type: "SELECT_CURRENCY",
            ...
            {bridgeType && bridgeType === "redeem" ? (
              <div className={"pt-4"}>
                <AddressInputPanel
                  ...
                  label={`Recipient ${
                    inputSymbol ? formatCoin(inputSymbol) : inputSymbol
                  } Address`}
                />
              </div>
            ...
            {!isLimitAction && (
              <div className={"pt-4"}>
                <AlertContainer>
                  <AlertText>
                    The liquidity of the cross chain bridge is insufficient,
                    please try again later. Balance:{" "}
                    {thousandBit(limitAmount, 2)} {inputSymbol}
                  </AlertText>
                </AlertContainer>
              </div>
            )}
